...
export async function GET(req) {
  try {
    const categories = await prisma.extension.findMany();
...
export async function POST(req) {
  try {
    ...
    const category = await prisma.extension.create({ data: { title,description, img } });
...
export async function PATCH(req) {
  ...
  try {
    ...
    const updatedCategory = await prisma.extension.update({
      where: { id },
...
export async function DELETE(req) {
  try {
    ...
    const deletedCategory = await prisma.extension.delete({ where: { id } });
